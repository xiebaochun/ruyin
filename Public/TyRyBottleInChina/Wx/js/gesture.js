var much = (function () {
    ...
    */
    ...
    toomuch.setCallback = function(callback){
        _callback = callback;
    }
    ...
    function onTouchStart(event) {
        ...
        if(event.touches.length === 1) {
            if(!element.dragging) {
                ...
                if(element.isDragable==true) handleDragStart(event.touches[0].clientX , event.touches[0].clientY);
            }
    ...
    function onTouchMove(event) {
        ...
        if(event.touches.length === 1) {
            if(element.isDragable==true) handleDragging(event.touches[0].clientX, event.touches[0].clientY);
        } else if(event.touches.length === 2) {
    ...
    function handleGesture(x1, y1, x2, y2) {
        if(element.isGesture) {
            ...
            _callback(element.currentScale);
    ...
    function onAnimationFrame() {
        ...
        elementHTML.style[prefixedTransform] = 'translate(' + element.posX + 'px,' + element.posY + 'px) rotate(' + element.currentRotation + 'deg) scale(' + element.currentScale + ')';     
            ...
            elementHTML.style[prefixedTransform] = 'translate(' + element.posX + 'px,' + element.posY + 'px) rotate(' + element.currentRotation + 'deg) scale(' + 1 + ')';
        }
